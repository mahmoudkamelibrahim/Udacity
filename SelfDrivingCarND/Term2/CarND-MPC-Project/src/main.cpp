      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (unsigned int i = 0; i < n_waypoints; i++ ) {
            ...
            ptsx_transformed( i ) = deltaX * cos( - psi ) - deltaY * sin( - psi );
            ptsy_transformed( i ) = deltaX * sin( - psi ) + deltaY * cos( - psi );
          }
          ...
          // Prediction after latency
          double x_delay = 0.0 + ( v * delay );
          double y_delay = 0.0;
          double psi_delay = 0.0 - ( v * delta / Lf * delay );
          double v_delay = v + a * delay;
          double cte_delay = cte0 + ( v * sin(epsi0) * delay );
          double epsi_delay = epsi0 - ( v * delta / Lf * delay);
