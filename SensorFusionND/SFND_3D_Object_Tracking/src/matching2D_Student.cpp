                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
  ...
  //TODO       cout << " (NN) with n=" << matches.size() << " matches in " << 1000 * t / 1.0 << " ms" << endl;
    ...
    { // k nearest neighbors (k=2)
        ...
        t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
 //TODO       cout << " (KNN) with n=" << knn_matches.size() << " matches in " << 1000 * t / 1.0 << " ms" <<  endl;
...
//TODO        cout << "# keypoints removed = " << knn_matches.size() - matches.size() << endl;
        // EOF TASK
    }
}
...
void descKeypoints(float &elapsed_time ,vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    else if (descriptorType.compare("ORB") == 0)
    {
      ...
      //cout << "Type >>>>>>>." << keypoints[0].octave << endl ;
